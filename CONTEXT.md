...
- The **Person** role is still exclusive — a **Person** is **Staff** or **Teaching Team**, never both — but nothing arranged now writes a **Teaching-Team Person**: both modes name their teachers as plain names, so the role is on its way out (retired once the old online table is dropped, a later step)
...
- **Class Records** are owed by whoever taught, and both modes now teach as names rather than signed-in People — so how they are counted and filed for a single-**Stream** online **Session** (the old "two professors, one per **Stream**, six per Session" no longer describes it) is deferred alongside the offline half (see Open questions). **Offline Sessions** already produce none, for the same reason
