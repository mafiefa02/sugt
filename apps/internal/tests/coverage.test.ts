...
describe("the Coverage payload", () => {
  ...
  it("is open to a non-Staff caller, who reads the same counts", async () => {
    /**
     ...
     * union — not a role check. `coverage` takes an ignored `_caller`, so a
     * hand-built non-Staff caller stands in for the retired Teaching Team member.
     */
    const staff = await signInAsStaff();
    await seedTwoClusters(staff.id);

    await expect(coverage(nonStaff())).resolves.toEqual(await coverage(staff));
  });
});
