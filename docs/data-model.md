...
- `better_auth` — `user`, `session`, `account`, `verification`. Generated by the Better Auth
  CLI, never hand-edited. Supabase's own `auth` schema is taken by GoTrue and owned by
  `supabase_auth_admin`, so this is a new schema rather than a shared one.
...
create table perjadin_evaluation (
  ...
  lodging      smallint not null check (lodging     between 1 and 10),
...
- **What else the Participant form asks for.** Right now: Class, five Ratings, comment, name.
